...
public class DonateActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void register(View view)
    {
        ...
        donorData.full_name = ((EditText) findViewById(R.id.editText_name)).getText().toString();
        donorData.phone = ((EditText) findViewById(R.id.editText_phone)).getText().toString();
        donorData.email = ((EditText) findViewById(R.id.editText_email)).getText().toString();
        donorData.addr = ((EditText) findViewById(R.id.editText_address)).getText().toString();

        donorData.bloodgrp = ((Spinner)findViewById(R.id.spinner_bloodgrp)).getSelectedItem().toString();
        donorData.city = ((Spinner)findViewById(R.id.spinner_city)).getSelectedItem().toString();
        donorData.area = ((Spinner)findViewById(R.id.spinner_area)).getSelectedItem().toString();

        // Validation of input data

        if(donorData.full_name.isEmpty() || donorData.phone.isEmpty() || donorData.email.isEmpty() ||  donorData.addr.isEmpty())
        {
            msg = "One or Multiple fields are Empty.\nCheck all Fields and try Again.";
            Toast.makeText(this,msg,Toast.LENGTH_LONG).show();
        }
        else if(!Patterns.EMAIL_ADDRESS.matcher( donorData.email).matches())
        {
            msg = "Invalid E-mail address.";
            Toast.makeText(this,msg,Toast.LENGTH_LONG).show();
        }
        else if( donorData.phone.length()<10)
        {
        ...
        else
        {
            ...
            dbHelper.insert(donorData, this);
            msg = "Successfully, registered as Donor.";
            Toast.makeText(this,msg,Toast.LENGTH_LONG).show();
            super.onBackPressed();
        }
